...
# Define the features to drop (data leakage/unnecessary)
features_to_drop = [
    "subject_id", "hadm_id_adm", "hadm_id_proc", "hadm_id_med", 
    "hadm_id_vs", "icd_version", "category", "description", "has_cancer_history"
]

# Drop unwanted columns if they exist
merged_df.drop(columns=[col for col in features_to_drop if col in merged_df.columns], errors="ignore", inplace=True)



for col in ["ref_range_upper", "hemid", "valuenum_y", "itemid"]:
    if col not in X_train.columns and col in merged_df.columns:
        X_train[col] = merged_df[col]
        X_test[col] = merged_df[col]
        print(f"✅ Restored missing feature: {col}")


# Define important features to keep
important_features = ["age", "valuenum_x", "ref_range_lower", "ref_range_upper", "valuenum_y"]


# Ensure only these features remain in X_train and X_test
